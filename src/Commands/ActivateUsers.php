...
class ActivateUsers extends Command implements SelfHandling {
  ...
  private $_user_ids;
    ...
    foreach($this->_user_ids as $user_id){
      $user = User::withTrashed()->find($user_id['id']);
      if($user){
        (Config::get('efront.lms')) ? $efront->ActivateUser($user) : $user->restore();
      }
    }
    return true;
	}
